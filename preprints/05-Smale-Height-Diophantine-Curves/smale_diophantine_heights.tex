...
The search for effective height bounds on algebraic curves of genus $g \ge 2$ remains one of the defining quests of modern arithmetic geometry. While Faltings' landmark theorem \cite{faltings1983} establishes the qualitative finiteness of $K$-rational points $C(K)$, its reliance on the Siegel-Roth method renders the resulting height bounds fundamentally ineffective.

As demonstrated in Section 2, the $abc$ conjecture of Masser and Oesterlé \cite{masser1985, oesterle1988} provides an overarching resolution through Elkies' theorem \cite{elkies1991}: the geometric branching of Belyi coverings $f: C \to \mathbb{P}^1$ produces an explicit uniform height bound $h(P) \le \mathcal{B}(C, K, \varepsilon)$, driven by the positive genus gap $2g - 2 > 0$. Unconditionally, the Chabauty-Coleman $p$-adic integration method \cite{chabauty1941, coleman1985} bounds the number of rational points when the Mordell-Weil rank satisfies $r < g$, while Kim's non-abelian Selmer program \cite{kim2005, kim2009} opens the path to higher rank curves via unipotent fundamental groups.

The machine-checked formalization developed in Section \ref{sec:lean_formalization} verifies the foundational algebraic invariants—including the positivity of the canonical divisor degree, the Chabauty dimension gap, and the $abc$ conductor bound—in Lean 4 with zero axioms. A complete formalization of the arithmetic intersection theory underpinning Faltings' modular heights constitutes a major objective for future interactive proof development.
